 ...
 *
 *   💭 Thinking      — live reasoning preview (beginning + end of thinking)
...
function reasoningSnippet(raw) {
  ...
  // Wrapped in <i>: thinking is the model's inner voice — italic sets it
  // clearly apart from the plain-text answer in the Reply section.
  if (chars.length <= REASONING_HEAD + REASONING_TAIL) {
    return "<i>" + esc(text) + "</i>";
  }
  const head = esc(chars.slice(0, REASONING_HEAD).join(""));
  const tail = esc(chars.slice(-REASONING_TAIL).join(""));
  return "<i>" + head + "\n… thinking continues …\n" + tail + "</i>";
}
