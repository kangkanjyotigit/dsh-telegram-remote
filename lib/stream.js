 ...
 *
 * Every header is colored-bullet + bold + underline; thinking content sits
 * in a <blockquote> card (accent-colored bar) with italic text.
 *
...
// Colored bullet + bold + underline: every section gets its own accent
// color (Telegram bots cannot set text colors, so the colored emoji bullets
// and the blockquote accent bar are the strongest color levers available).
const THINKING_HEADER = "🔵 <b><u>THINKING</u></b>";
const TOOLS_HEADER = "🟢 <b><u>TOOLS</u></b>";
const REPLY_HEADER = "🟣 <b><u>REPLY</u></b>";
const DIVIDER = "─────── ⋆⋅☆⋅⋆ ───────";
...
export function setupStreaming(state) {
  ...
  if (!engine.started) {
    ...
    (async () => {
      try {
        const frames = api.events.mux({ rpcId: "tg-mux-" + Math.random().toString(36).slice(2, 10), payload: {} }, engine.abort.signal);
        ...
        if (!engine.abort?.signal.aborted) state.log("mux stream ended: " + error.message);
      }
    })();
  }
...
function finalizeReply(state, stream, content) {
  ...
  // Thinking is already shown live in the bubble while the model works —
  // the final edit is the clean answer only (no duplicated thinking block).
...
function buildPreview(stream) {
  ...
  if (stream.text) {
    ...
    sections.push("🔵 <b><u>THINKING</u></b>…");
  }
