...
async function handleText(state, chatId, userId, text, messageId) {
  ...
  try {
    ...
    try {
      await state.bot.send(chatId, "⚠️ " + friendly + "\n<small>If you keep seeing this, /help or ask the owner.</small>");
    } catch {}
  }
}
...
export function apply(ctx, config) {
  ...
  try {
    ...
    for (const older of botRegistry) {
      ...
      try { older.stop(); state.log("stopped previous poller (live reload)"); } catch {}
    }
      ...
      bot.getMe().then(async (me) => {
        ...
        if (config.notifyOnStartup && !(globalThis.__dshTgStarted ?? false)) {
          ...
          for (const chatId of targets) {
            state.push(chatId, "🟢 <b>DSH Remote online</b>\n─────── ⋆⋅☆⋅⋆ ───────\n<small>pid " + process.pid + " · " + esc(process.cwd()) + "\n/help for everything</small>");
          }
        }
      }).catch((error) => {
    ...
    const watchdog = ctx.setInterval(() => {
      try {
        if (!holdLock) {
          if (tryAcquire()) {
            state.log("bot lock acquired after retry");
            startBot();
          }
          return;
        }
        if (state.bot.stopped) return;
        if (state.bot.lastPollAt > 0 && Date.now() - state.bot.lastPollAt > 180_000 && !state.bot.loop) {
          state.log("watchdog: restarting poll loop");
          setupDispatch(state);
        }
      } catch {}
    }, 30_000);
    ...
    return () => {
      ...
      try { ctx.clearInterval(watchdog); } catch {}
      try {
        if (holdLock) {
          const raw = readFileSync(lockPath, "utf8").trim();
