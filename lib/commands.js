...
async function cmdStatus(r) {
  ...
  try {
    ...
    if (active) {
      ...
      lines.push("   📌 <i>current: " + esc(title ?? shortId(active)) + "</i>");
    }
  } catch {
...
/** Flatten provider groups into { provider, id, name, efforts } rows. */
function flattenCatalog(catalog) {
  const out = [];
  for (const group of catalog ?? []) {
    const provider = group.id ?? group.group?.id ?? "";
    const models = group.models ?? group.group?.models ?? [];
    for (const model of models) {
      ...
      out.push({
        ...
        efforts: (model.reasoning?.efforts ?? []).map((e) => e.id),
      });
    }
  }
  return out;
...
async function cmdModels(r) {
  ...
  if (flat.length === 0) return "No models are available right now — check your API keys in the web Models page.";
  let currentId = "";
  try {
    const sessionId = await r.state.resolveSessionId(r.chatId, "");
    if (sessionId) {
      const info = await callApi(r.ctx, "sessions", "models", { sessionId });
      currentId = info?.current?.model ?? "";
    }
  } catch {}
  const cs = r.state.chatState(r.chatId);
  cs.lastModels = flat.map((m) => ({ provider: m.provider, model: m.id }));
  cs.lastModelsAt = Date.now();
  r.state.saveState();
  const lines = ["🧠 <b>Pick a model</b>", "tap a button, or reply with a <b>number</b>:"];
  const rows = [];
  const shown = flat.slice(0, 18);
  shown.forEach((m, i) => {
    const n = i + 1;
    const on = m.id === currentId;
    lines.push((on ? "✅ " : "") + "<b>" + n + ".</b> <code>" + esc(m.id) + "</code>" + (m.efforts.includes("max") ? " ⭐" : ""));
    const cmd = "/model " + m.id;
    const cb = cmd.length <= 64 ? cmd : "md:" + n;
    const label = (on ? "✅ " : "") + n + ". " + (m.id.length > 26 ? m.id.slice(0, 24) + "…" : m.id);
    rows.push([{ text: label, callback_data: cb }]);
  });
  if (flat.length > shown.length) lines.push("<i>+ " + (flat.length - shown.length) + " more — type /model &lt;id&gt;</i>");
  rows.push([{ text: "⭐ Deepest thinking (max)", callback_data: "/model max" }]);
  return { text: truncate(lines.join("\n"), 3500), keyboard: { inline_keyboard: rows } };
}

async function cmdModel(r) {
  const args = r.args;
  const sessionId = await r.state.resolveSessionId(r.chatId, "");
  if (!sessionId && args.length === 0) {
    const info = await currentModelInfo(r.ctx, r.state, r.chatId);
    const lines = ["<b>This chat</b>: none open yet — tap 💬 New chat"];
    if (info.def?.provider) lines.push("<b>Default</b>: " + esc(info.def.provider) + " / " + esc(info.def.model));
    lines.push("", "Tap 🧠 Model to pick one, or send a message to start chatting.");
    return lines.join("\n");
  }
  if (args.length === 0) {
    const info = await currentModelInfo(r.ctx, r.state, r.chatId);
  ...
  if (/^\d+$/.test(first)) {
    const list = r.state.chatState(r.chatId).lastModels ?? [];
    const pick = list[Number(first) - 1];
    if (!pick) return "That number isn't on the last model list — tap 🧠 Model again.";
    return setSessionModel(r, sessionId, { provider: pick.provider, model: pick.model });
  }
  ...
  // also accepts provider/model as a single token.
  const catalog = await getCatalog(r.ctx);
  const flat = flattenCatalog(catalog);
  let provider = null;
  let model = null;
  ...
  if (raw.includes("/") && args.length === 1) {
    const slash = raw.indexOf("/");
    provider = raw.slice(0, slash);
    model = raw.slice(slash + 1);
  } else if (args.length >= 2 && (catalog.some((g) => (g.id ?? g.group?.id) === args[0]) || flat.some((m) => m.provider === args[0]))) {
    provider = args[0];
    model = args[1];
    effort = args[2];
  } else {
  ...
  if (!provider && model) {
    const hit = flat.find((m) => m.id === model) ?? flat.find((m) => m.id.toLowerCase() === String(model).toLowerCase());
    provider = hit?.provider ?? null;
    if (hit) model = hit.id;
  }
  if (!provider || !model) return "I couldn't find that model — tap 🧠 Model and pick one from the list.";
...
async function setSessionModel(r, sessionId, { provider, model, reasoningEffort }) {
  if (!sessionId) {
    const settings = r.ctx.get("settings");
    if (!settings || typeof settings.replace !== "function") return "Open a chat first (tap 💬 New chat), then pick a model.";
    if (!provider || !model) return "Tap 🧠 Model and pick one from the list.";
    await settings.replace("agent-default-model", {
      provider,
      model,
      ...(reasoningEffort ? { reasoningEffort } : {}),
    });
    return "✅ Next chat will use <b>" + esc(model) + "</b>\nTap 💬 New chat or just send a message.";
  }
  let current = {};
  try {
  ...
  if (!payload.provider || !payload.model) return "This chat has no model yet — tap 🧠 Model and pick one.";
  try {
    await callApi(r.ctx, "sessions", "selectModel", payload);
  } catch (error) {
    const msg = String(error.message ?? error);
    if (/image input|does not accept image|does not support image/i.test(msg)) {
      return "⚠️ This chat already has photos, so it needs a vision model.\nPick one that can see images, or tap 💬 New chat and switch there.";
    }
    return "⚠️ Couldn't switch model: " + esc(truncate(msg, 220));
  }
  return "✅ Switched to <b>" + esc(payload.model) + "</b>\n<i>" + esc(payload.provider) + (payload.reasoningEffort ? " · " + esc(payload.reasoningEffort) : "") + "</i>\nSend a message to use it.";
}
...
async function cmdSessions(r) {
  ...
  const lines = ["📋 <b>Your chats</b>", "reply with a <b>number</b> to open one:"];
  show.forEach((item, index) => {
    const mark = item.sessionId === active ? " 👈" : "";
    const status = item.running ? "🏃" : item.blank ? "⬜" : "💤";
    const title = item.projections?.values?.title;
    // Shorten the path to its last segment — full paths clutter mobile chats.
    const folder = item.cwd ? basename(item.cwd) : "";
    lines.push(
      (index + 1) + ". " + status + " <b>" + esc(title ?? "untitled") + "</b>" + mark + "\n   <i>" + fmtAge(item.updatedAt) + (folder ? " · " + esc(folder) : "") + "</i>",
    );
  });
  if (sessions.length > show.length) lines.push("<i>…and " + (sessions.length - show.length) + " more</i>");
...
async function cmdQueue(r) {
  ...
  items.forEach((item, index) => {
    ...
    lines.push((index + 1) + ". <i>" + tag + "</i> " + esc(truncate(item.text, 90)));
  });
  lines.push(
    "─────── ⋆⋅☆⋅⋆ ───────",
...
async function cmdNotify(r) {
  ...
  if (arg === "status") {
    return "🔔 <b>Notify:</b> <code>" + state.notify + "</code>" + (state.sessionId ? " <i>(active session " + shortId(state.sessionId) + ")</i>" : "");
  }
